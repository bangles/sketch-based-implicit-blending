...
TemplateScene::TemplateScene(QOpenGLShaderProgram *program){
    this->m_program = program;
    grid = new Grid(3 * 44, m_program);
    m_template = new Template(m_program);
    userPoints = new UserPoints(m_program);
    m_processor = new Processor(*m_template);
    m_pipeline = new Pipeline(*m_template);
...
void TemplateScene::setViewportAspectRatio(float aspectRatio){
    m_camera.setViewportAspectRatio(aspectRatio);
}
